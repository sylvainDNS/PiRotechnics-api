...
export const initChannels = () => {
  if (channels.length == 0) {
    const init = (min, max, switched) => {
      const tmpChannels = Array.from({ length: max - min }).map((_, index) => {
        return {
...
export const launch = (number, time) => {
  ...
  const set = state => {
    ...
    if (channel.switch == null) channel.switch.write(state)
  }
...
export const isActivated = number => {
  ...
  const isSwitch =
    channel.switch == null ? true : channel.switch.read() == 1 ? true : false
